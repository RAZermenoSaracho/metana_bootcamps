...
| --- | --- |
| **ID** | **Behaviour** |
| FE-1 | Create an ERC20 token contract using OpenZeppelin libraries, with a name, symbol, and number of decimals of your choice. The constructor should accept a parameter to initialize the MAX\_SUPPLY. |
| FE-2 | Implement the supply cap for the ERC20 token contract by overriding the \_update() function. |
| FE-3 | An account with the MINTER\_ROLE should have the ability to mint tokens. You can use the AccessControlDefaultAdminRules library from OpenZeppelin to implement this. |
...
| FE-6 | The token sale contract should be granted the MINTER\_ROLE in the ERC20 token contract. |
